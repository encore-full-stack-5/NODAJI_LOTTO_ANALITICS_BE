...
async def get_vertical_chart(index, data):
    df_sorted = sort_lotto(index, data)
    all_values = df_sorted.values.flatten()
    counts = pd.Series(all_values).value_counts().reindex(range(1, 46), fill_value=0)
...
async def get_lotto_result_by_group(index, data):
    ...
    all_values = df_sorted.values.flatten()
    counts = pd.Series(all_values).value_counts().reindex(range(1, 46), fill_value=0)
    total_lotto_result_by_group = []
    for i in range(1, 46, 10):
        total_lotto_result_by_group.append(counts.loc[i:i+9].sum())
